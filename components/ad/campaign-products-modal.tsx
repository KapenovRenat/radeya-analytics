...
function fmtDate(s: string) {
  return new Date(s).toLocaleDateString("ru-RU", { day: "numeric", month: "short", timeZone: "UTC" });
}
...
function InlineDropdown({ value, options, onSave }: {
  ...
  return (
    <div ref={ref} className="relative inline-block">
      ...
      {open && (
        <div className="absolute left-0 top-full z-[60] mt-1 min-w-[110px] rounded-[var(--radius)] border border-[var(--border)] bg-[var(--surface-elev)] py-1 shadow-lg">
          {options.map((o) => (
            <button key={o.value}
...
type WeekTotal = {
  weekStart: string; weekEnd: string; granularity?: string;
...
const COMPARE_METRICS: CompareMetric[] = [
  ...
  { key: "orders",      label: "Заказы",           fmt: (v) => String(Math.round(v)),              higherIsBetter: true  },
...
function ProductCompareModal({
  ...
  // Aggregate products totals per period
  const totals: WeekTotal[] = periods.map((p) => {
    let spent = 0, impressions = 0, orders = 0;
    let drrSum = 0, ctrSum = 0, convCartSum = 0, convFavSum = 0, avgClickSum = 0, count = 0;
  ...
  const delta = (metric: keyof WeekTotal, first: number, last: number) => {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="mb-5 flex items-start justify-between">
          <div>
            <h2 className="text-[15px] font-semibold text-[var(--text)]">Сравнение периодов</h2>
            <p className="mt-0.5 text-[12px] text-[var(--text-dim)]">
              Суммарные показатели по {products.length} товарам
            </p>
          </div>
          ...
          <table className="w-full text-[12px]">
            <thead>
              <tr className="border-b border-[var(--border)]">
                <th className="py-2 pr-4 text-left text-[10px] font-semibold uppercase tracking-[0.06em] text-[var(--text-dim)] min-w-[130px]">
                  Метрика
                </th>
                {periods.map((p) => (
            ...
            <tbody>
              {COMPARE_METRICS.map((metric) => {
                ...
                const d = periods.length >= 2 ? delta(metric.key, first, last) : null;
    ...
    <>
      ...
      {!collapsed && sorted.map((p, pi) => {
        ...
        return (
          <tr key={p.id} className={cn("border-b border-[var(--border)] transition-colors hover:bg-white/[0.02]", pi % 2 !== 0 && "bg-white/[0.01]")}>
            <td className="sticky left-0 z-10 bg-[var(--bg)] px-4 py-2 min-w-[220px] max-w-[280px]">
      ...
      {!collapsed && catTotals.length > 0 && (
        <tr className="border-b border-[var(--border)] bg-white/[0.01]">
          ...
          <td colSpan={6} />
          {catTotals.map((t) => [
            <td key={`${t.weekStart}-tot-spent`} className="border-l border-[var(--border)] px-2 py-1.5 text-right text-[10px] font-semibold tabular-nums text-[#60a5fa]">{fmt(t.spent)}</td>,
            <td key={`${t.weekStart}-tot-impr`} className="px-2 py-1.5 text-right text-[10px] font-semibold tabular-nums text-[var(--text-dim)]">{fmt(t.impressions)}</td>,
            <td key={`${t.weekStart}-tot-tc`} className="px-2 py-1.5" />,
            <td key={`${t.weekStart}-tot-cl`} className="px-2 py-1.5" />,
            <td key={`${t.weekStart}-tot-orders`} className="px-2 py-1.5 text-right text-[10px] font-semibold tabular-nums text-[var(--emerald)]">{t.orders} зак.</td>,
...
interface Props {
  ...
  campaignId: string;
  campaignName: string;
  /** Initial week selection from parent — user can change inside the modal */
  selectedWeeks: string[];
  onClose: () => void;
}

export function CampaignProductsModal({ storeId, campaignId, campaignName, selectedWeeks, onClose }: Props) {
  ...
  // Own week selection — driven by periods returned from products API (campaign-specific)
  const [localSelectedWeeks, setLocalSelectedWeeks] = useState<string[]>([]);
  ...
  // Load ALL data for this campaign once — periods come from the API (campaign-specific).
  // Week filtering is done client-side so no extra round-trips.
  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/kaspi/ad/${storeId}/products?campaignId=${encodeURIComponent(campaignId)}`);
      const data = await res.json();
      const allPeriods: Period[] = data.periods ?? [];
      setProducts(data.products ?? []);
      setPeriods(allPeriods);

      // Build available weeks from actual data (this campaign only)
      const weeks: WeekOption[] = allPeriods.map((p) => ({
        weekStart: p.weekStart,
        weekEnd: p.weekEnd,
        granularity: (p.granularity ?? "week") as "week" | "day",
      }));
      setAvailableWeeks(weeks);

      // Initial selection: use parent's selectedWeeks if they exist in this campaign's data,
      // otherwise default to latest 4 available periods
      setLocalSelectedWeeks((prev) => {
        // prev is [] on first load — use selectedWeeks prop as hint
        const hint = prev.length > 0 ? prev : selectedWeeks;
        const valid = hint.filter((s) => weeks.some((w) => w.weekStart === s));
  ...
  }, [storeId, campaignId, selectedWeeks]);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setUploadResults([{ filename: "—", error: "Ошибка сети" }]);
    } finally {
  ...
  // Client-side period filter (no re-fetch needed — all data already loaded)
  const displayPeriods: Period[] = localSelectedWeeks.length === 0
    ? periods
    : periods.filter((p) => localSelectedWeeks.includes(p.weekStart));

  // Filter + group products by search
  ...
  const grandTotals = displayPeriods.map((p) => {
    let spent = 0, impressions = 0, orders = 0;
  ...
  return (
    <>
      ...
      >
        <div
          className="flex flex-col w-full max-w-[95vw] max-h-[92vh] rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] shadow-2xl"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-[var(--border)] px-5 py-3.5 shrink-0">
            <div>
              <h2 className="text-[14px] font-semibold text-[var(--text)]">{campaignName}</h2>
              <p className="mt-0.5 text-[11px] text-[var(--text-dim)]">
                {loading ? "Загрузка..." : `${filtered.length} товаров · ${displayPeriods.length}${displayPeriods.length < periods.length ? ` из ${periods.length}` : ""} ${periods.length === 1 ? "период" : "периодов"}`}
              </p>
            </div>
            <button onClick={onClose} className="rounded p-1.5 text-[var(--text-dim)] hover:bg-white/10 transition-colors">
              <X className="h-4 w-4" />
            </button>
          </div>

          {/* Filter bar */}
          <div className="flex flex-wrap items-center gap-2.5 border-b border-[var(--border)] px-5 py-2.5 shrink-0">
            {/* Week selector — shows only weeks with actual product data for this campaign */}
            <WeekSelector
              weeks={availableWeeks}
              selected={localSelectedWeeks}
              onChange={setLocalSelectedWeeks}
              loading={loading && availableWeeks.length === 0}
            />
            ...
            {displayPeriods.length >= 2 && (
              <Button variant="ghost" size="sm" onClick={() => setCompareOpen(true)}>
                ...
                Сравнить {displayPeriods.length} пер.
              </Button>
            )}
          ...
          {/* Upload panel */}
          {uploadOpen && (
            <div className="border-b border-[var(--border)] bg-[var(--bg-subtle)] px-5 py-3 shrink-0">
              <div
                ...
                className={cn(
                  "flex cursor-pointer flex-col items-center gap-2 rounded-[var(--radius)] border-2 border-dashed p-5 transition-colors",
              ...
              >
                ...
                <p className="text-[12px] text-[var(--text-dim)]">Перетащи CSV «По товарам» или кликни · можно несколько недель</p>
              ...
              {uploadFiles.length > 0 && (
                <div className="mt-2 flex justify-end">
                  <Button variant="primary" size="sm" onClick={handleUpload} disabled={uploadUploading}>
          ...
          <div className="flex-1 overflow-auto">
            {loading && (
              <div className="flex items-center gap-2 p-8 text-[13px] text-[var(--text-dim)]">
                <Loader2 className="h-4 w-4 animate-spin" /> Загрузка товаров...
              </div>
            )}
            {error && (
              <div className="m-4 rounded-[var(--radius)] border border-[var(--red)]/30 bg-[var(--red-soft)] p-4 text-[13px] text-[var(--red)]">{error}</div>
            )}
            {!loading && !error && filtered.length === 0 && (
              <div className="p-12 text-center text-[13px] text-[var(--text-dim)]">
                {products.length === 0 ? "Товары для этой кампании не найдены" : "Нет совпадений"}
              </div>
            )}
              ...
              <table className="w-full text-[12px]" style={{ minWidth: "max-content" }}>
                <thead className="sticky top-0 z-20">
                  ...
                  <tr className="border-b border-[var(--border)] bg-[var(--bg-subtle)]">
                    <th className="sticky left-0 z-30 bg-[var(--bg-subtle)] px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-[0.06em] text-[var(--text-dim)] min-w-[220px]">Товар</th>
                    ...
                    <th className="px-3 py-2.5 text-center text-[10px] font-semibold uppercase tracking-[0.06em] text-[var(--text-dim)] whitespace-nowrap">Улучшить</th>
                  ...
                  {/* Sub-header */}
                  <tr className="border-b border-[var(--border)] bg-[var(--bg-subtle)]">
                    <th className="sticky left-0 z-30 bg-[var(--bg-subtle)] px-4 py-1" />
                    <th colSpan={6} />
                    {displayPeriods.map((p) =>
                ...
                {grandTotals.length > 0 && (
                  <tfoot>
                    <tr className="border-t-2 border-[var(--border-strong)] bg-[var(--bg-subtle)]">
                      ...
                      <td colSpan={6} />
                      {grandTotals.map((t) => [
                        <td key={`${t.weekStart}-gt-spent`} className="border-l border-[var(--border)] px-2 py-2 text-right text-[11px] font-semibold tabular-nums text-[#60a5fa]">{fmt(t.spent)}</td>,
                        <td key={`${t.weekStart}-gt-impr`} className="px-2 py-2 text-right text-[11px] font-semibold tabular-nums text-[var(--text-dim)]">{fmt(t.impressions)}</td>,
                        <td key={`${t.weekStart}-gt-tc`} className="px-2 py-2" />,
                        <td key={`${t.weekStart}-gt-cl`} className="px-2 py-2" />,
                        <td key={`${t.weekStart}-gt-orders`} className="px-2 py-2 text-right text-[11px] font-semibold tabular-nums text-[var(--emerald)]">{t.orders} зак.</td>,
      ...
      {/* Comparison modal (z-[60] so it renders on top of products modal) */}
      {compareOpen && (
        <ProductCompareModal
          products={filtered}
          periods={displayPeriods}
