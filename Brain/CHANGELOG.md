...
- ✅ Дата сдачи = `courierTransmissionPlanningDate − 2 дня` (`send-order.ts`, `handoffStr`); расчёт по дням склада удалён.
- ✅ Расписание авто-отправки: пн–сб 08:00–17:00 Almaty (`withinSendWindow()` в cron); вне окна синк идёт, отправка/отмены ждут утра/понедельника. Ручная кнопка без ограничений.
- ✅ AGENTS.md: правило давать коммент для коммита после каждой задачи.
