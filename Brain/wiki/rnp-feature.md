---
aliases: [раздел РНП, реклама в приложении]
tags: [feature, rnp, разработка]
created: 2026-05-28
updated: 2026-06-02
---

# Фича: Раздел РНП в приложении

> Планируемый раздел `/stores/[id]/rnp` для еженедельного контроля рекламных компаний прямо в приложении вместо Excel.

## Содержание

### Цель

Заменить ручной Excel-файл РНП встроенным разделом приложения.
Загрузка CSV из Kaspi → автоматические метрики и оценки → недельная динамика.

### Статус

**В разработке** — план согласован 2026-05-28, все 4 страницы реализованы и работают. Идёт доработка деталей.

---

### БД — 4 таблицы

| Таблица | Назначение                         |
|---|------------------------------------|
| `ad_campaigns` | Справочник компаний                |
| `ad_weekly_stats` | Метрики компании по неделям        |
| `ad_products` | Справочник товаров внутри компании |
| `ad_product_stats` | Метрики товаров по неделям         |

**`ad_campaigns`:**
`id, store_id, name, status, improve_card, has_reviews, has_discount, in_stock, has_video`

**`ad_weekly_stats`:**
`id, campaign_id, week_start, week_end, is_monthly_total, impressions, spent, daily_budget, target_click, avg_click, orders, revenue, drr_pct, ctr_pct, conv_cart_pct, conv_fav_pct, rating`

**`ad_products`:**
`id, campaign_id, name, category, status, improve_card, has_reviews, has_discount, in_stock, has_video`

**`ad_product_stats`:**
`id, product_id, campaign_id, week_start, week_end, impressions, spent, target_click, avg_click, orders, revenue, drr_pct, ctr_pct, conv_cart_pct, conv_fav_pct, rating`

---

### Загрузка данных

**Объём файлов в месяц:**
- «По компаниям»: 1 CSV/неделю × 4-5 недель + 1 итоговый = ~6 файлов
- «По товарам»: 1 CSV/компанию/неделю × ~50 компаний × 5 недель = ~250 файлов

**Два отдельных bulk upload блока:**

**Блок 1 — «По компаниям»** (до ~6 файлов)
- Каждый файл = одна неделя или итог за месяц
- Авто-детект периода из дат внутри файла
- Парсинг: название компании, все метрики недели

**Блок 2 — «По товарам»** (до ~250 файлов)
- Каждый файл = одна кампания за одну неделю
- Авто-детект: название кампании + период из содержимого файла
- Парсинг: товар, категория, метрики недели

**Оба блока:**
- Мультиселект файлов
- Превью перед сохранением
- Upsert — повторная загрузка безопасна

---

### Архитектура — слой абстракции (для будущего API)

```
Источник               Ingestion layer            БД
───────────────        ──────────────────         ──────────────────────
CSV upload       ──→                              rnp_campaigns
                       ingestCampaignWeek()  →    rnp_weekly_stats
Kaspi API (future)→                               rnp_products
                                                  rnp_product_weekly_stats
```

Когда откроется Kaspi Marketing API — добавляем новый входной модуль, БД и аналитика не меняются.

---

### UI — 4 страницы (реализовано)

**Сводка `/stores/[id]/ad/summary`**
- WeekSelector мультиселект (по умолчанию последние 4 недели)
- KPI стрип 6 карточек: Расход · Показы · Заказы · Выручка · ДРР% · CTR%
- Графики (Recharts): Расход по неделям (bar) · Показы по неделям (bar) · ДРР% (line) · Заказы (bar) · Распределение оценок (pie) · Топ-10 по расходу (bar)

**По кампаниям `/stores/[id]/ad/campaigns`**
- WeekSelector с удалением недели (trash → inline confirm) + поиск + сортировка + кнопка «Сравнить N нед.»
- Таблица: sticky Название · Статус · флаги (5 шт.) + горизонтальный скролл
- На каждую неделю 11 колонок: Расход · Показы · Бюджет/д · Целев.клик · Ср.клик · Заказы/ДРР% · CTR% · Конв→корз% · Конв→изб% · Выручка · Оценка
- Выручка: read-only (amber) для недельных периодов; editable для monthly total
- `<tfoot>` — итого по расходу/показам/заказам за каждую неделю
- Modality: «Сравнить N нед.» — таблица агрегатов + Δ дельта + спарклайны

**По товарам `/stores/[id]/ad/products`**
- Те же фильтры + выбор кампании + кнопка «Загрузить» (inline upload panel)
- Группировка по категориям, сворачиваемые секции
- На каждую неделю 10 колонок: Расход · Показы · Целев.клик · Ср.клик · Заказы/ДРР% · CTR% · Конв→корз% · Конв→изб% · Выручка · Оценка
- Итого по категории + grand total `<tfoot>`

**Загрузка `/stores/[id]/ad/upload`**
- Два drag-and-drop блока: «По кампаниям» и «По товарам»
- Превью файлов, upsert-безопасно, результат после загрузки
- Раздел удаления: «Удалить все данные» (cascade) / «Удалить только статистику»

---

### API Routes

```
GET    /api/kaspi/ad/[storeId]/summary                         — KPI + графики
GET    /api/kaspi/ad/[storeId]/campaigns?weeks[]=&from=&to=    — кампании с недельной статистикой
GET    /api/kaspi/ad/[storeId]/products?campaignId=&weeks[]=   — товары с недельной статистикой
GET    /api/kaspi/ad/[storeId]/weeks                           — список доступных недель (desc)
POST   /api/kaspi/ad/[storeId]/upload/campaigns                — загрузка CSV кампаний
POST   /api/kaspi/ad/[storeId]/upload/products                 — загрузка CSV товаров
PATCH  /api/kaspi/ad/[storeId]/campaigns   body: {type, id, field, value}
PATCH  /api/kaspi/ad/[storeId]/products    body: {type, id, field, value}
DELETE /api/kaspi/ad/[storeId]/reset?target=all|stats|week     — очистка данных
POST   /api/kaspi/ad/[storeId]/periods                         — создать ручной период-плейсхолдер
GET|POST|DELETE /api/kaspi/ad/[storeId]/overview               — обзорный отчёт (daily store data)
GET|POST|DELETE /api/kaspi/ad/[storeId]/tg-recipients          — получатели Telegram
POST   /api/kaspi/ad/[storeId]/report                          — генерация и отправка отчёта в Telegram
```

### Новые таблицы БД (добавлены позже)

| Таблица | Назначение |
|---|---|
| `ad_store_overview` | Store-level данные по дням из «Обзорного отчёта» Kaspi |
| `tg_recipients` | Получатели Telegram для отчёта (name, chatId) |

---

### Навигация (сайдбар)

```
📈 Реклама РНП
  ├── Сводка
  ├── Обзор           ← новый: store-level обзорный отчёт, сравнение недель
  ├── По кампаниям
  ├── Загрузка CSV
  └── Отчёт в Telegram ← новый: отправка отчёта в Telegram
```

(«По товарам» убрана из навигации — открывается через кнопку «Товары» в каждой кампании)

---

### Рабочий процесс пользователя (актуальный)

1. Каждый понедельник скачать CSV из Kaspi кабинета (По кампаниям + Обзорный отчёт за неделю)
2. Загрузить кампании через «Загрузка CSV»; Обзорный отчёт через «Обзор»
3. Смотреть **Сводку** — графики и KPI по неделям
4. Смотреть **Обзор** — store-level метрики, сравнение недель
5. Идти в **По кампаниям** — анализировать оценки, сравнивать недели, смотреть товары
6. Каждый день скачивать «Обзорный отчёт» за вчера → отправлять через **Отчёт в Telegram**

## Связано с

- [[kaspi-rnp]] — что такое РНП, структура данных
- [[kaspi-metrics]] — формулы и нормы оценок
- [[project]] — общая архитектура приложения

## Источник

- Планирование с Ренатом, 2026-05-28
